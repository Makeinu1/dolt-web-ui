...
1. ヘッダーの **[⋮] → 🔍 全テーブル検索** をクリック
2. キーワードを入力し、必要なら **メモも検索する** を ON にして検索します
3. 結果一覧から行をクリックすると該当テーブルへ移動し、対象行にジャンプします
...
1. **[⋮] → 🔍 全テーブル検索** を開きます
2. キーワードを入力して検索します。必要なら **メモも検索する** を ON にします
3. 結果をクリックすると該当テーブルに切り替わり、対象行へジャンプします
4. backend が integrity failure を返した場合は partial result を表示せず、そのままエラーとして扱います
...
│   │   │   ├── SearchModal/         # 全テーブル検索（値 + 任意で memo）
│   │   │   ├── TableGrid/           # AG Grid + 右クリック + ドラフト可視化 + memo marker + 行Undo
│   │   │   └── common/
...
| メソッド | パス | 説明 |
|----------|------|------|
| GET | `/api/v1/memo` | 単一セルのメモ取得 |
| GET | `/api/v1/memo/map` | テーブル内のメモ座標マップ取得 |
...
| メソッド | パス | 説明 |
|----------|------|------|
| GET | `/api/v1/search` | 値 + メモ横断検索。partial を返さず fail-loud |
