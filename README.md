...
### 8. バージョン比較（[⋮] → Compare Versions...）

1. 比較したい2つのバージョン（`merged/*` タグ）を選択して **Compare**
2. DB全テーブルの変更サマリー（追加/更新/削除の件数）を確認
3. テーブルを選択して行レベルのdiffにドリルダウン（AG Gridで表示、フルスクリーン対応）
...
│   │   │   ├── TableGrid/           # AG Grid + 右クリックメニュー + ドラフト可視化 + コメントマーカー
│   │   │   └── common/
...
| POST | `/api/v1/comments` | コメント追加（即時コミット）。MainGuard 適用 |
| POST | `/api/v1/comments/delete` | コメント削除（即時コミット）。MainGuard 適用 |
