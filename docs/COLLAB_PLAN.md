...
**最終更新**: 2026-03-27
**状態**: `POST /api/v1/sync` エンドポイント公開・「mainと同期」ボタン追加済み。E2E テスト7本追加・全94本PASS。docs 同期完了。
...
| **Compare-UX** | ✅ landed | 比較モードを横並びグリッド表示に全面改修（`CompareResultModal.tsx`）。E2E テスト `compare-mode.spec.ts`（24本）追加。docs 同期（CLAUDE.md / README.md / manual-test-checklist / test-strategy） |
| **Sync-endpoint** | ✅ landed | `POST /api/v1/sync` 公開（`handler/sync.go` 新規）。⋮ メニュー「⬇ mainと同期」追加（App.tsx）。`branchRefreshKey` を `getTables` 依存配列に追加。E2E テスト `sync.spec.ts`（7本）追加。全94本PASS。 |
...
- **行比較モード 実機確認**: `manual-test-checklist.md` セクション 18（MT-18-*）のブラウザ手動試験を実施すること
